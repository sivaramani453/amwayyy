...
resource "aws_lb_listener" "tcp_listener" {
  ...
  port              = "${lookup(var.tcp_listeners[count.index], "port")}"
  protocol          = "${lookup(var.tcp_listeners[count.index], "protocol")}"
  count             = "${var.tcp_listeners_count}"

  default_action {
    target_group_arn = "${aws_lb_target_group.target_group.*.id[lookup(var.tcp_listeners[count.index], "target_group_index", 0)]}"
    type             = "forward"
  }
}
