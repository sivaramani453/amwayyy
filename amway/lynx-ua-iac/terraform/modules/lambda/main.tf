...
resource "aws_lambda_function" "main" {
  ...
  tags = "${merge(map("Name", "${var.function_name}-lambda-main"), var.custom_tags_common)}"
}
...
resource "aws_lambda_function" "from_bucket" {
  ...
  tags = "${merge(map("Name", "${var.function_name}-lambda-from-bucket"), var.custom_tags_common)}"
}
