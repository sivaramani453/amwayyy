...
resource "null_resource" "deploy_external_dns" {
  ...
  depends_on = [
    null_resource.check_api,
    local_file.external_dns_manifest,
  ]
...
data "template_file" "cluster_autoscaler_config" {
  template = file(
    "${path.module}/manifests_templates/cluster-autoscaler/kubernetes-autoscaler.tpl",
  )
...
data "template_file" "cluster_autoscaler_priority_configmap" {
  template = file(
    "${path.module}/manifests_templates/cluster-autoscaler/autoscaler-priority.tpl",
  )
...
resource "null_resource" "deploy_cluster_autoscaler" {
  depends_on = [
    local_file.cluster_autoscaler_config,
    null_resource.priority_class,
    local_file.cluster_autoscaler_priority_configmap,
  ]
...
data "template_file" "aws_alb_ingress_config" {
  template = file(
    "${path.module}/manifests_templates/aws-alb-ingress/alb-ingress-controller.tpl",
  )
...
resource "null_resource" "deploy_aws_alb_ingress" {
  depends_on = [
    null_resource.check_api,
    local_file.aws_alb_ingress_config,
  ]
...
resource "null_resource" "copy_manifests" {
  depends_on = [
    ...
    local_file.external_dns_manifest,
    null_resource.deploy_aws_alb_ingress,
  ]
