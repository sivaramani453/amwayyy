...
resource "aws_db_subnet_group" "vip-reports-rds-subnet-group" {
  name = "vip-reports_subnet_groups"
  subnet_ids = data.terraform_remote_state.core.outputs.database_subnets
  description = "vip-reports subnetgroup"
  }
...
resource "aws_db_instance" "vip_reports" {
  ...
  username               = "root"
  password               = var.root_password
  db_subnet_group_name   = aws_db_subnet_group.vip-reports-rds-subnet-group.name
