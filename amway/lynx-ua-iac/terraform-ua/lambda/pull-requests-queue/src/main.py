...
def get_merging_pull_requests(prs):
    ...
    for pr in prs:
        if pr.labels.get("merge it"):
            if pr.labels.get("add to queue") or pr.labels.get(
                    "add to queue urgent"):
                res.append(pr)
    return res
...
def get_jobs_for_pulls(g, git_repo, ctx, *args):
    ...
    for pr in locked_base_branch_prs:
        ...
        username = g.get_user(pr.login)
        msg = "Base branch: {base} is locked. {user}, please check your {url}".format(
            base=pr.base, user=username, url=pr.href_url)
    ...
    for pr in not_mergeable_prs:
        ...
        # identify user to make msg more user friendly
        username = g.get_user(pr.login)
        msg = "{base}: {user}, please fix conflicts in {url}".format(
            base=pr.base, user=username, url=pr.href_url)
    ...
    for pr in failed_checks_prs:
        ...
        # identify user to make msg more user friendly
        username = g.get_user(pr.login)
        msg = "{base}: {user}, please fix status checks in {url}".format(
            base=pr.base, user=username, url=pr.href_url)
    ...
    # so if somebody added add to queue label one should ensure it is actually ready to be merged
    ...
    for pr in not_approved_prs:
        ...
        username = g.get_user(pr.login)
        msg = "{base}: {user}, please make sure {url} is approved".format(
            base=pr.base, user=username, url=pr.href_url)
        job = Job(pr=pr,
                  action="notify_and_relabel",
                  label=PR_FAILED_LABEL,
...
def lambda_handler(event, context):
    ...
    if ctx.env == "eu":
        ctx.notificator = [
            SkypeNotificator(to=config.skype.chan["eu"],
