locals {
  ...
  frankfurt_kubernetes_subnet_cidrs  = ["10.130.160.0/26", "10.130.160.64/26", "10.130.160.128/26"]
  frankfurt_gitlab_ci_subnet_cidrs   = ["10.130.160.192/28", "10.130.160.208/28", "10.130.160.224/28"]
  frankfurt_database_subnets         = ["10.130.160.240/28", "10.130.161.0/28", "10.130.161.16/28"]
  frankfurt_lambda_subnet_cidrs      = ["10.130.161.32/27", "10.130.161.64/27", "10.130.161.96/27"]
  frankfurt_public_subnets           = ["10.130.161.128/27", "10.130.161.160/27", "10.130.161.192/27"]
  frankfurt_rancher_subnet_cidrs     = ["10.130.161.224/28", "10.130.161.240/28", "10.130.162.0/28"]
  frankfurt_rancher_alb_subnet_cidrs = ["10.130.162.16/28", "10.130.162.32/28", "10.130.162.48/28"]
  frankfurt_address_validation_cidrs = ["10.130.162.64/28", "10.130.162.80/28", "10.130.162.96/28"]
...
resource "aws_subnet" "frankfurt_gitlab_ci" {
  provider          = "aws.frankfurt"
  count             = "${length(local.frankfurt_gitlab_ci_subnet_cidrs)}"
  vpc_id            = "${module.frankfurt_vpc.vpc_id}"
  cidr_block        = "${element(local.frankfurt_gitlab_ci_subnet_cidrs, count.index)}"
  availability_zone = "${element(local.frankfurt_azs, count.index)}"

  tags = {
    Name      = "gitlab-ci-private-${element(local.frankfurt_subnet_postfixes, count.index)}"
    Terraform = "true"
  }
}
...
resource "aws_route_table_association" "frankfurt_private" {
  provider = "aws.frankfurt"
  count    = "${length(concat(local.frankfurt_kubernetes_subnet_cidrs, local.frankfurt_lambda_subnet_cidrs, local.frankfurt_gitlab_ci_subnet_cidrs, local.frankfurt_rancher_subnet_cidrs, local.frankfurt_rancher_alb_subnet_cidrs, local.frankfurt_address_validation_cidrs))}"

  subnet_id      = "${element(concat(aws_subnet.frankfurt_kubernetes.*.id, aws_subnet.frankfurt_lambda.*.id, aws_subnet.frankfurt_gitlab_ci.*.id, aws_subnet.frankfurt_rancher.*.id, aws_subnet.frankfurt_rancher_alb.*.id, aws_subnet.frankfurt_address_validation.*.id), count.index)}"
...
resource "aws_vpn_connection" "frankfurt_production" {
  ...
  type                = "ipsec.1"
  tunnel1_inside_cidr = "169.254.0.56/30"
  tunnel2_inside_cidr = "169.254.0.60/30"
