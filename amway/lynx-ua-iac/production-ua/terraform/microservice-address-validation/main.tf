...
resource "null_resource" "provisioning_the_cluster" {
  ...
  provisioner "local-exec" {
    ...
    ansible-playbook ./ansible/bootstrap-cluster.yml -u centos -i ./ansible/hosts --extra-vars "solr_efs_mountpoint_src='${aws_route53_record.efs_urls.name}' solr_additional_opts='-Djdbc.postgresql.host=${element(split(":", module.rds_pgsql.this_db_instance_endpoint),0)} -Djdbc.postgresql.user=${var.microservice_db_user} -Djdbc.postgresql.pass=${var.microservice_db_pass} -Dadapter_auth_credentials=${var.adapter_auth_user}:${var.adapter_auth_pass} -Devent_sender_1_data=https://${var.address_validation_url}/python/adapter/kz/solr_job_done -Devent_sender_2_data=https://${var.address_validation_url}/python/adapter/ru/solr_job_done -Devent_sender_3_data=https://${var.address_validation_url}/python/adapter/ua/solr_job_done'" --private-key="$PATH_TO_SSH_KEY";
  EOT
