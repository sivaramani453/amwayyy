resource "aws_iam_instance_profile" "ec2_instance_iam_profile" {
  name = "db-create-snapshot-${terraform.workspace}-iam-profile"
...
module "ec2_instance_iam_role" {
  ...
  role_name         = "db-create-snapshot-${terraform.workspace}-iam-role"
...
module "ec2_instance_ec2_access_policy" {
  ...
  name        = "DBCS-InstProf-EC2-${terraform.workspace}"
  path        = "/"
    ...
    "Statement": [
        {
            ...
            "Resource": [
                "arn:aws:ec2:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:volume/${aws_ebs_volume.db.id}",
