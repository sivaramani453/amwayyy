...
instance_types = [
    {'type': 't3.micro',    'max_price': '0.01'},
    {'type': 't3a.micro',   'max_price': '0.01'},
...
class Queued(Request):
    ...
    def process(self, body):
        ...
        name_tag = [{"Key": "Name", "Value": "github-actions-runner-v2-" + str(body['workflow_job']['id'])}]
        ...
        job_workflow_tags = [
            ...
            {"Key": "Repository", "Value": self.di.config.git.repo}
        ]
        ...
        self.di.logger.debug(userdata.format_map(userdata_vals));

        try:
            self.di.logger.info('Attempting to request spot instance...')
            requestResult = self.requestInstance(userdata_vals, name_tag, job_workflow_tags, body)
            response = requestResult['Response']
            typeWanted = requestResult['RequestedType']

        except Exception as e:
            self.di.logger.error(traceback.format_exc())
            self.di.logger.error(e)
            return Request.returnJson(500, {'message': 'Unable to request spot instances'})
...
# I need to fetch spot request object 1st, and only then use its create_tags method
        ...
        full_repo_name = "{0}/{1}".format(self.di.config.git.org, self.di.config.git.repo)
        ...
        self.di.logger.info("Join token were obtained in github: {0}***{1}".format(
                    join_token[:2], join_token[-2:]))
        self.di.logger.info('Putting GH worker join information in SSM for a just started host')

        try:
            self.put_ssm_params(instanceId, full_repo_name, join_token, typeWanted)
        except Exception as e:
        ...
        return Request.returnJson(200, {'message': 'Workers should be booting right now.'})

    def requestInstance(self, userdata_vals, name_tag, job_workflow_tags, body):
                ...
                while current_maxprice < 1:
                    ...
                    try:                    
                        response = self.di.awsClient.run_instances(
                                ...
                                EbsOptimized =  True,
                                SubnetId = self.di.config.aws.subnet,
                        ...
                        if e.response['Error']['Code'] == 'SpotMaxPriceTooLow':
                            ...
                            if matched:
                                ...
                                current_maxprice = float(matched.group(1))
                            else:
                                # PANIK!!!
                                current_maxprice = str(current_maxprice + 0.005 + (1 - current_maxprice) / 2)
    ...
    def put_ssm_params(self, id, repo, token, type):
