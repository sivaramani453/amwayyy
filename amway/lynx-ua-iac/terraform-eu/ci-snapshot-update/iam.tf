resource "aws_iam_instance_profile" "ec2_instance_iam_profile" {
  name = "ci-update-snapshot-${terraform.workspace}-iam-profile"
...
module "ec2_instance_iam_role" {
  ...
  role_name         = "ci-update-snapshot-${terraform.workspace}-iam-role"
  role_requires_mfa = false

  custom_role_policy_arns = [
    module.ec2_instance_ssm_policy.arn,
...
module "ec2_instance_ssm_policy" {
  source  = "terraform-aws-modules/iam/aws//modules/iam-policy"
  version = "~> 3.0"

  name        = "CIUS-InstProf-SSM-${terraform.workspace}"
  path        = "/"
  description = "Policy for the ec2 instance profile to access ssm"
...
module "ec2_instance_ec2_access_policy" {
  ...
  name        = "CIUS-InstProf-EC2-${terraform.workspace}"
  path        = "/"
    ...
    "Statement": [
        {
            ...
            "Resource": [
                "arn:aws:ec2:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:volume/${aws_ebs_volume.ci_db.id}",
