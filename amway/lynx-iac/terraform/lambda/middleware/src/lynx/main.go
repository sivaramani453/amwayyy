...
func PostHandler(req events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	if payload.Action == "labeled" {
		...
		case "merge it":
			if payload.Sender.Login != ServiceUser1 && payload.Sender.Login != ServiceUser2 {
...
func getPlan(p *PullRequestPayload) (string, error) {
	...
	case (len(labelMap) == 0 && (p.Sender.Login == ServiceUser1 || p.Sender.Login == ServiceUser2)) || labelMap["restart"]:
