...
resource "aws_route_table_association" "frankfurt_private" {
  count = "${length(concat(local.additional_spot_workers_subnets, local.spot_workers_subnets, local.infra_subnets))}"

  subnet_id      = "${element(concat(aws_subnet.ondemand_workers.*.id, aws_subnet.spot_workers.*.id, aws_subnet.infra_subnets.*.id), count.index)}"
