resource "aws_route53_record" "zookeeper_node_urls" {
  ...
  name    = "${terraform.workspace}-zookeeper-node-${count.index}.ru.eia.amway.net"
  ttl     = "300"
  type    = "A"
...
resource "aws_route53_record" "solr_node_urls" {
  ...
  zone_id = "${data.terraform_remote_state.core.route53_zone_id}"
  name    = "${terraform.workspace}-solr-node-${count.index}.ru.eia.amway.net"
  ttl     = "300"
  type    = "A"
...
resource "aws_route53_record" "efs_urls" {
  zone_id = "${data.terraform_remote_state.core.route53_zone_id}"
  name    = "${aws_efs_file_system.efs_address_validation_fs.id}.efs.ru.eia.amway.net"
  ttl     = "300"
  type    = "A"
