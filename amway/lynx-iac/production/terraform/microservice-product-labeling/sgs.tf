module "pl_nodes_sg" {
  ...
  tags = "${merge(map("Name", "pl-nodes-${terraform.workspace}-sg"), local.amway_common_tags)}"
}
...
module "alb_nodes_sg" {
  ...
  tags = "${merge(map("Name", "alb-nodes-${terraform.workspace}-sg"), local.amway_common_tags)}"
}
