module "ec2_product_labeling_postgresql_instance" {
  ...
  volume_tags = "${merge(map("ServiceType", "product_labeling-postgresql-instance"), local.custom_tags_common, local.custom_tags_volume)}"
  tags        = "${merge(map("Schedule", "running"), local.custom_tags_common, local.custom_tags_instance)}"
}
...
resource "aws_ebs_volume" "db" {
  ...
  type              = "gp3"
  size              = 100
  tags              = "${merge(map("ServiceType", "product_labeling-postgresql-instance"), local.custom_tags_common, local.custom_tags_volume)}"
}
...
resource "aws_efs_file_system" "efs_product_labeling_fs" {
  tags = "${merge(local.custom_tags_common, local.custom_tags_volume)}"
}
