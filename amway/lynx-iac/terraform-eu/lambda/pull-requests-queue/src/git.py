...
class GitClient:
    ...
    def get_detailed_pull_request(self, pull_num, git_repo):
        ...
        print("Generate detailed PR", git_pull_request, "Repo", git_repo)
...
def generate_pr_obj(git_repo, pr):
    ...
    p = PullRequest(git_pull_request=pr,
                    ...
                    base=pr.base.ref,
                    url=pr.html_url,
                    href_url='[{num}]({url})'.format(url=pr.html_url, num=pr.number),
                    head=pr.head.ref,
                    approved=False,
                    rejected=False,
    ...
    # Sort last review by users
    reviews_status = {}
    reviews_status.update({x.user.login: x.state for x in pr.get_reviews()})
    print("Reviewers last status", reviews_status)
    # Get review state
    reviews = set([x for x in list(reviews_status.values())])
    print("Review conclusion", reviews)
    if APPROVED in reviews:
        p.approved = True
    if CHANGES_REQUESTED in reviews:
        p.rejected = True
