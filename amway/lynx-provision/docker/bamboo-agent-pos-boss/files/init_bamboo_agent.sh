...
echo """
import docker,os
name = docker.from_env().containers.get(os.environ['HOSTNAME']).attrs['Name']
number = name.split('_')[-1]
print(number)
""" > get_number.py
NUMBER_NAME=$(sudo python3 get_number.py)
PREFIX=/docker_agents_pos_boss/$HOST_HOSTNAME
PARAMETER_NAME=${PREFIX}_docker_agent_${NUMBER_NAME}
IDS=$(aws ssm get-parameter --name ${PARAMETER_NAME} --query "Parameter.[Value]" 2>/dev/null --output text)
AGENT_ID=${IDS%%,*}
AGENT_UID=${IDS##*,}
