...
until [ "$n" -ge 12 ]
do
    RUNNER_TOKEN=$($PYTHON $AWS ssm get-parameter --name $SSM_PARAM_TOKEN --with-decryption --query 'Parameter.Value' --output text --region $REGION) 2>&1 >/dev/null
    if [ "$?" -ne 0 ]; then
...
until [ "$n" -ge 12 ]
do
    RUNNER_REPO=$($PYTHON $AWS ssm get-parameter --name $SSM_PARAM_REPO --with-decryption --query 'Parameter.Value' --output text --region $REGION) 2>&1 >/dev/null
    if [ "$?" -ne 0 ]; then
...
until [ "$n" -ge 12 ]
do
    RUNNER_LABELS=$($PYTHON $AWS ssm get-parameter --name $SSM_PARAM_TYPE --with-decryption --query 'Parameter.Value' --output text --region $REGION) 2>&1 >/dev/null
    if [ "$?" -ne 0 ]; then
